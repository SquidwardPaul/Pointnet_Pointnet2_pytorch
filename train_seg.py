...
def parse_args():
    ...
    parser.add_argument('--num_classes', type=int, default=16, help='Classes Number [default: 16]')
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        ''' 调整 learning_rate、momentum，对应修改optimizer、classifier '''
        # 在每个周期开始时，调整 学习率，修改 optimizer
        ...
        for i, data in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            classifier = classifier.train() # 使用PyTorch进行训练时,一定注意要把实例化的model指定train,表示启用 BatchNormalization 和 Dropout
